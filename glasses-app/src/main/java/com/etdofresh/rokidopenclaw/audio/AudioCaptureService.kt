...
/**
 * Captures raw PCM audio from the device microphone (mono, 16kHz, 16-bit)
 * and emits fixed-size frames as a [Flow] of [ByteArray].
 *
 * Each frame is ~100ms of audio (3200 bytes at 16kHz/16bit/mono).
...
class AudioCaptureService(private val context: Context) {
    ...
    companion object {
        ...
        /**
         * Frame size in bytes for one 100ms chunk at 16kHz/16bit/mono:
         * 16000 samples/s × 2 bytes/sample × 0.1s = 3200 bytes
         */
